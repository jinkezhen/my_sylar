...
template<class T>
bool ReadFromStreamWithSpeed(std::ifstream& is, const std::vector<T>& v, const uint64_t& speed = -1) {
    if (is) {
...
template<class T>
bool ReadFromStreamWithSpeed(std::ifstream& is, const T& v, const uint64_t& speed = -1) {
    if (is) {
...
//将src按size块大小切割，并存入dst
template<class T>
void Slice(std::vector<std::vector<int>>& dst, const std::vector<T>& src, size_t size) {
    //计算src的剩余待处理元素的大小
